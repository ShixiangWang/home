...
`as` 和 `wrap` 是 Rcpp 很重要的两个转换函数，这里 `as` 将输入参数 `xs` 由 R 输入的 SEXP 类型转换为整型，而 `wrap` 将 c++ 得到的整型结果封装为 SEXP 类型，从而可以使得这个创建的函数可以被 `.Call()` 调用，完成 c++ 的计算与输入输出的相互传递。
